...
def plot_multi_bar(x, data, **kwargs):
    ...
    for idx in range(np.shape(data)[0]):
        ax.bar(x, data[idx], color=kwargs.get('color')[idx], edgecolor=kwargs.get('color')[idx], bottom=np.sum(data[:idx], axis=0) if idx != 0 else None)

    ax.set_title(kwargs.get('title'))
    ax.set_xlabel(kwargs.get('xlabel'))
    ax.set_ylabel(kwargs.get('ylabel'))

    if kwargs.get('xticks') is not None: ax.set_xticks(kwargs.get('xticks'))
    if kwargs.get('yticks') is not None: ax.set_yticks(kwargs.get('yticks'))
    ...
    if kwargs.get('labels') is not None: ax.legend(kwargs.get('labels'), loc='upper right', fontsize='small', bbox_to_anchor=kwargs.get('legend_anchor'))
